  ...
  --设置炮台倍数
     for i=1,8 do
       ...
       if nil ~= cannon then
          ...
          pos = CannonSprite.getPos(self._dataModel.m_reversal,cannonPos)
...
function GameLayer:onSubFishCreate( dataBuffer )
    ...
    if fishNum >= 1 then
    	for i=1,fishNum do
         ...
         if FishCreate.nFishType == g_var(cmd).FishType.FishType_JinLong or FishCreate.nFishType == g_var(cmd).FishType.FishType_YinQianNianGui or FishCreate.nFishType == g_var(cmd).FishType.FishType_JinQianNianGui then
            local tips 

            if FishCreate.nFishType == g_var(cmd).FishType.FishType_JinLong then
                tips = "金龙"
            elseif FishCreate.nFishType == g_var(cmd).FishType.FishType_YinQianNianGui then
                tips = "银色千年龟摇钱树"
            else
                tips = "金色千年龟摇钱树"
            end
        ...
        for i=1,catchNum do
           ...
           --获取炮台视图位置
             ...
             if catchData.wChairID == self.m_nChairID then   --自己
                ...
                if scoreType == g_var(cmd).SupplyType.EST_YuanBao then
