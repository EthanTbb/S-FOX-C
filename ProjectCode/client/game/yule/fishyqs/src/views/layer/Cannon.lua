...
function Cannon:initWithUser(userItem)
	...
	if self.m_nChairID > 5 then
		nMyNum = 1
	else
		nMyNum = self.m_nChairID / 3
	end

	local nPlayerNum = 0

	if userItem.wChairID > 5 then
		nPlayerNum = 1
	else
		nPlayerNum = userItem.wChairID / 3
	end

	if self._dataModel.m_reversal and self.m_pos <= 5 then 
...
function Cannon:ShowSupply( data )
	...
	elseif nSupplyType == g_var(cmd).SupplyType.EST_Gift then
		...
		---[[
		local cannonPos = Cannon.getPos(self._dataModel.m_reversal,self.m_nChairID)
