...
// 文字数インジケータビット长(バージョングループ别, {S, M, L})
...
bool CQR_Encode::EncodeData(int nLevel, int nVersion, bool bAutoExtent, int nMaskingNo, char* lpsSource, int ncSource)
{
    ...
    // データ长が指定されていない场合は lstrlen によって取得
    ...
    else
    {
        ...
        {
            if (bAutoExtent)
                m_nVersion = nEncodeVersion; // バージョン(型番)自动扩张
            else
                return 0; // 容量オーバー
        }
    }
    ...
    int nDataCwIndex = 0; // データコードワード处理位置
...
//// 用  途：エンコード时バージョン(型番)取得
//// 引  数：调查开始バージョン、エンコードデータ、エンコードデータ长
//// 戻り值：バージョン番号（容量オーバー时=0）
//
...
// 用  途：入力データエンコード
// 引  数：入力データ、入力データ长、バージョン(型番)グループ
// 戻り值：エンコード成功时=1
...
bool CQR_Encode::EncodeSourceData(char* lpsSource, int ncLength, int nVerGroup)
{
    ...
    /////////////////////////////////////////////////////////////////////////
    // 邻接する英数字モードブロックと数字モードブロックの并びをを条件により结合

    int ncSrcBits, ncDstBits; // 元のビット长と单一の英数字モードブロック化した场合のビット长

    int nBlock = 0;

    while (nBlock < m_ncDataBlock - 1)
    {
        int ncJoinFront, ncJoinBehind; // 前后８ビットバイトモードブロックと结合した场合のビット长
        int nJoinPosition = 0; // ８ビットバイトモードブロックとの结合：-1=前と结合、0=结合しない、1=后ろと结合

        // “数字－英数字”または“英数字－数字”の并び
        ...
        {
            // 元のビット长と单一の英数字モードブロック化した场合のビット长を比较
            ...
            {
                // 前后に８ビットバイトモードブロックがある场合、それらとの结合が有利かどうかをチェック
                if (nBlock >= 1 && m_byBlockMode[nBlock - 1] == QR_MODE_8BIT)
                {
                    ...
                    if (ncJoinFront > ncDstBits + GetBitLength(QR_MODE_8BIT, m_nBlockLength[nBlock - 1], nVerGroup))
                        ncJoinFront = 0; // ８ビットバイトモードブロックとは结合しない
                }
                else
                    ncJoinFront = 0;
                ...
                if (nBlock < m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_8BIT)
                {
                    ...
                    if (ncJoinBehind > ncDstBits + GetBitLength(QR_MODE_8BIT, m_nBlockLength[nBlock + 2], nVerGroup))
                        ncJoinBehind = 0; // ８ビットバイトモードブロックとは结合しない
                }
                else
                    ncJoinBehind = 0;
                ...
                {
                    // 前后两方に８ビットバイトモードブロックがある场合はデータ长が短くなる方を优先
    ...
    while (nBlock < m_ncDataBlock - 1)
    {
        ...
        // 前に８ビットバイトモードブロックがある场合、重复するインジケータ分を减算
        if (nBlock >= 1 && m_byBlockMode[nBlock - 1] == QR_MODE_8BIT)
            ncDstBits -= (4 + nIndicatorLen8Bit[nVerGroup]);

        // 后ろに８ビットバイトモードブロックがある场合、重复するインジケータ分を减算
        if (nBlock < m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_8BIT)
            ncDstBits -= (4 + nIndicatorLen8Bit[nVerGroup]);
        ...
        if (ncSrcBits > ncDstBits)
        {
            if (nBlock >= 1 && m_byBlockMode[nBlock - 1] == QR_MODE_8BIT)
            {
                // 结合するブロックの前にある８ビットバイトモードブロックを结合
                m_nBlockLength[nBlock - 1] += m_nBlockLength[nBlock];
            ...
            if (nBlock < m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_8BIT)
            {
                // 结合するブロックの后ろにある８ビットバイトモードブロックを结合
                m_nBlockLength[nBlock + 1] += m_nBlockLength[nBlock + 2];
    ...
    /////////////////////////////////////////////////////////////////////////
    // ビット配列化
    int ncComplete = 0; // 处理济データカウンタ
    unsigned short wBinCode;

    m_ncDataCodeWordBit = 0; // ビット单位处理カウンタ
...
void CQR_Encode::GetRSCodeWord(unsigned char* lpbyRSWork, int ncDataCodeWord, int ncRSCodeWord)
{
    ...
    for (i = 0; i < ncDataCodeWord; ++i)
    {
        if (lpbyRSWork[0] != 0)
        {
            unsigned char nExpFirst = byIntToExp[lpbyRSWork[0]]; // 初项系数より乘数算出

            for (j = 0; j < ncRSCodeWord; ++j)
            {
                // 各项乘数に初项乘数を加算（% 255 → α^255 = 1）
...
void CQR_Encode::FormatModule()
{
    ...
    if (m_nMaskingNo == -1)
    {
        ...
        SetFormatInfoPattern(m_nMaskingNo); // フォーマット情报パターン配置
        ...
        for (i = 1; i <= 7; ++i)
        {
            ...
            SetFormatInfoPattern(i); // フォーマット情报パターン配置
...
void CQR_Encode::SetCodeWordPattern()
{
    ...
    int nCoef_x = 1; // ｘ轴配置向き
    int nCoef_y = 1; // ｙ轴配置向き
    ...
    for (i = 0; i < m_ncAllCodeWord; ++i)
    {
        for (j = 0; j < 8; ++j)
        {
            ...
            } while (m_byModuleData[x][y] & 0x20); // 机能モジュールを除外
...
void CQR_Encode::SetMaskingPattern(int nPatternNo)
{
    ...
    for (i = 0; i < m_nSymbleSize; ++i)
    {
        for (j = 0; j < m_nSymbleSize; ++j)
        {
            if (!(m_byModuleData[j][i] & 0x20)) // 机能モジュールを除外
            {
                bool bMask;
