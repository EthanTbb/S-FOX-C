// QR_Encode.h : CQR_Encode クラス宣言およびインターフェイス定义
...
typedef struct tagQR_VERSIONINFO
{
    ...
    int ncAllCodeWord; // 总コードワード数

    // 以下配列添字は误り订正率(0 = L, 1 = M, 2 = Q, 3 = H) 
    int ncDataCodeWord[4];	// データコードワード数(总コードワード数 - ＲＳコードワード数)

    int ncAlignPoint;	// アライメントパターン座标数
    int nAlignPoint[6];	// アライメントパターン中心座标

    RS_BLOCKINFO RS_BlockInfo1[4]; // ＲＳブロック情报(1)
    RS_BLOCKINFO RS_BlockInfo2[4]; // ＲＳブロック情报(2)
